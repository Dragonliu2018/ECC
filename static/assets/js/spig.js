...
jQuery(document).ready(function ($) {
    ...
    if ( isindex==true ) { //如果是主页
        var now = (new Date()).getHours();
        if (now >= 4 && now < 12) {
            showMessage(visitor + "Hi! Good morning! Welcome to our website!", 6000);
        } else if (now >= 12 && now < 18) {
            showMessage(visitor + 'Hi! Good afternoon! Welcome to our website!', 6000);
        } else if (now >= 18 && now < 23) {
            showMessage(visitor + 'Hi! Good evening! Welcome to our website!', 6000);
        } else{
            showMessage(visitor + '夜深了，注意休息哦！', 6000);
        }
    }
    else {

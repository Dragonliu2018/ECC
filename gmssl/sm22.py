...
default_ecc_table = {  #标准推荐的参数
    'n': 'FFFFFFFEFFFFFFFFFFFFFFFFFFFFFFFF7203DF6B21C6052B53BBF40939D54123',
    'p': 'FFFFFFFEFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF00000000FFFFFFFFFFFFFFFF',
    'g': '32c4ae2c1f1981195f9904466a39c9948fe30bbff2660be1715a4589334c74c7'      #G（x,y）
...
#        print("计算椭圆曲线上的点 C1 = kG = (%s, %s)"% (C1[0:self.para_len],C1[self.para_len:2*self.para_len]))
        xy = self._kg(int(k, 16), self.public_key)
        x2 = xy[0:self.para_len]
        y2 = xy[self.para_len:2*self.para_len]
#        print("计算椭圆曲线上的点 kPB = (x2,y2) = (%s, %s)" % (x2, y2))
        ml = len(msg)
...
#            print("计算C2 = M XOR t = ",C2)
 ...
 #           print("计算C3 = SM3(x2||M||y2) = ",C3)
            if self.mode:
  #              print("输出密文C = C1||C2||C3 = ",bytes.fromhex('%s%s%s' % (C1, C3, C2)))
                return bytes.fromhex('%s%s%s' % (C1, C3, C2)),k,C1,x2,y2,t,C2,C3
            else:
   #             print("输出密文C = C1||C2||C3 = ", bytes.fromhex('%s%s%s' % (C1, C2, C3)))
                return bytes.fromhex('%s%s%s' % (C1, C2, C3)),k,C1,x2,y2,t,C2,C3
    ...
    def decrypt(self, data):
        ...
        if int(t, 16) == 0:
            ...
            return bytes.fromhex(M),x2,y2,t,u,C3,M
